...
async function fixture(t) {
  ...
  const incident = await service.createIncident(payload, meta);
...
test('four independent patients and encounters preserve one operational incident and identity timeline', async t => {
  const { service: s, incident: i, calls } = await fixture(t);
  const cases = [];
  for (let n = 0; n < 4; n++) {
    const c = await s.createPatientCase(i.incident_id, n === 2 ? { temporary_label: 'Unknown patient' } : {}, { ...meta, idempotencyKey: `case-${n}` });
    assert.equal(c.patient_case_id, `PCR-00000${n + 1}`); assert.equal(c.patient_sequence, n + 1);
    assert.equal((await s.createPatientCase(i.incident_id, n === 2 ? { temporary_label: 'Unknown patient' } : {}, { ...meta, idempotencyKey: `case-${n}` })).patient_case_id, c.patient_case_id);
    ...
    await s.linkPatientToPatientCase(c.patient_case_id, { openemr_patient_id: patient.patient_id, verification_status: n === 2 ? 'provisional' : 'verified' }, meta);
  ...
  assert.equal(new Set((await s.listPatientCases(i.incident_id)).map(c => c.openemr_patient_id)).size, 4);
  assert.equal(calls.filter(c => c.patient_case_id).length, 4);
  assert.equal((await s.incidents.listAll()).length, 1);
  assert.equal((await s.listSyncIntents()).filter(x => x.operation === 'createIncidentMirror').length, 1);
  await assert.rejects(() => s.getPatientLink(i.incident_id), /patient_case_id is required/);
  ...
  await assert.rejects(() => s.linkPatientToPatientCase(unknown.patient_case_id, { openemr_patient_id: 'identified', verification_status: 'verified' }, meta), /reconciliation/);
  const reconciled = await s.reconcilePatientCaseIdentity(unknown.patient_case_id, { verified_patient_id: 'identified', reason: 'Identity confirmed' }, meta);
  ...
  assert.equal((await s.db.queryAll('PRAGMA foreign_key_check')).length, 0);
  ...
  assert.equal((await s.getIncident(i.incident_id)).closure_ready, true);
  await s.assertClosureAllowed(i.incident_id);
});
...
for (const [count, complete, ready] of [[1,1,true],[2,2,true],[2,1,false],[4,4,true]]) test(`closure considers all cases: ${complete}/${count}`, async t => {
  ...
  for (let n = 0; n < count; n++) {
    const c = await s.createPatientCase(i.incident_id, {}, meta);
    await s.linkPatientToPatientCase(c.patient_case_id, { verification_status: 'verified', openemr_patient_id: `p-${n}` }, meta);
  ...
  assert.equal((await s.getIncident(i.incident_id)).closure_ready, ready);
  if (ready) await s.assertClosureAllowed(i.incident_id); else await assert.rejects(() => s.assertClosureAllowed(i.incident_id), /closure metadata/);
});
...
test('assignment context, ambiguity, lead validation and responsibility transfer', async t => {
  ...
  await assert.rejects(() => s.createPatientCase(i.incident_id, { lead_clinician_id: staff[1].staff_id }, meta), /Lead clinician/);
  const b = await s.createAssignment(i.incident_id, { vehicle_id: 'AMB-002', crew_ids: [staff[1].staff_id], reason: 'dispatch' }, meta);
  await s.updateAssignment(b.assignment_id, { action: 'confirm_assignment' }, meta);
  await assert.rejects(() => s.createPatientCase(i.incident_id, {}, meta), /Multiple active assignments/);
  const moved = await s.changePatientCaseAssignment(c.patient_case_id, { assignment_id: b.assignment_id }, meta);
  ...
  await assert.rejects(() => s.assertClosureAllowed(i.incident_id), /active assignments/);
  await assert.rejects(() => s.setPatientCaseStatus(c.patient_case_id, 'Closed', meta), /Invalid patient case transition/);
});
...
test('Stage 5 migration retains every legacy link field and fresh schema has no orphan links', async t => {
  ...
  const { patient_case_id: pId, ...p } = await upgraded.queryOne('SELECT * FROM patient_case_patient_links');
  const { patient_case_id: eId, ...e } = await upgraded.queryOne('SELECT * FROM patient_case_encounter_links');
  ...
  assert.equal((await upgraded.queryOne("SELECT openemr_patient_id FROM patient_case_patient_links WHERE incident_id='INC-000002'")).openemr_patient_id, 'patient');
  assert.equal((await upgraded.queryAll('PRAGMA foreign_key_check')).length,0); upgraded.db.close();
});

test('patient case IDs never reuse deleted IDs and idempotency rejects changed incident/payload', async t => {
  const { service: s, incident: i } = await fixture(t);
  const first = await s.createPatientCase(i.incident_id, {}, { ...meta, idempotencyKey: 'stable' });
  await assert.rejects(() => s.createPatientCase(i.incident_id, { temporary_label: 'different' }, { ...meta, idempotencyKey: 'stable' }), /different request/);
  const second = await s.createPatientCase(i.incident_id, {}, meta);
  await s.db.execute(`DELETE FROM patient_cases WHERE patient_case_id='${second.patient_case_id}'`);
  const third = await s.createPatientCase(i.incident_id, {}, meta);
  assert.equal(first.patient_case_id, 'PCR-000001'); assert.equal(third.patient_case_id, 'PCR-000003');
  await assert.rejects(() => s.db.execute(`UPDATE patient_cases SET patient_sequence=1 WHERE patient_case_id='${third.patient_case_id}'`), /UNIQUE/);
  await assert.rejects(() => s.createPatientCase('INC-999999', {}, meta), /not found/);
});

test('legacy wrappers resolve a single case and relinking before encounter preserves audit history', async t => {
  const { service: s, incident: i } = await fixture(t);
  const first = await s.linkPatientToIncidentContext(i.incident_id, { verification_status: 'provisional', temporary_label: 'Unknown' }, meta);
  assert.equal(first.patient_case_id, 'PCR-000001');
  await s.linkPatientToIncidentContext(i.incident_id, { verification_status: 'verified', openemr_patient_id: 'known' }, meta);
  assert.equal((await s.getPatientLink(i.incident_id)).openemr_patient_id, 'known');
  const history = await s.db.queryAll("SELECT * FROM audit_logs WHERE action='link_patient'");
  ...
  assert.equal((await s.getEncounterByIncident(i.incident_id)).encounter_id, e.encounter_id);
});

test('concurrent encounter creation writes once and patient relinking is blocked during remote write', async t => {
  const { service: s, incident: i } = await fixture(t);
  const c = await s.createPatientCase(i.incident_id, {}, meta);
  await s.linkPatientToPatientCase(c.patient_case_id, { verification_status: 'verified', openemr_patient_id: 'patient' }, meta);
  ...
  await assert.rejects(() => s.linkPatientToPatientCase(c.patient_case_id, { verification_status: 'verified', openemr_patient_id: 'other' }, meta), /reconciliation/);
...
test('responsibility transfer preserves native links and stock authorship follows each current vehicle', async t => {
  ...
  const assignments = [];
  for (let n = 1; n <= 2; n++) {
    const staff = await s.createPersonnel({ staff_id: `STAFF-00${n}`, display_name: 'Crew', role: 'Paramedic', operational_status: 'Available', home_station: 'Test' }, meta);
    await s.createVehicle({ vehicle_id: `AMB-00${n}`, callsign: `Crew ${n}`, vehicle_type: 'Ambulance', operational_status: 'Available', service_status: 'Serviceable', home_station: 'Test' }, meta);
    const a = await s.createAssignment(i.incident_id, { vehicle_id: `AMB-00${n}`, crew_ids: [staff.staff_id], reason: 'dispatch' }, meta);
    await s.updateAssignment(a.assignment_id, { action: 'confirm_assignment' }, meta); assignments.push(a);
  }
  const c = await s.createPatientCase(i.incident_id, { assignment_id: assignments[0].assignment_id }, meta);
  await s.linkPatientToPatientCase(c.patient_case_id, { verification_status: 'verified', openemr_patient_id: 'patient' }, meta);
  const e = await s.createEncounterForPatientCase(c.patient_case_id, { care_started_at: '2026-09-06T10:00:00Z', presenting_complaint: 'Exercise' }, meta);
  await s.createStockItem({ stock_item_id: 'ITEM-001', name: 'Test dressing', category: 'Consumable', unit_of_measure: 'each', item_type: 'Consumable' }, meta);
  for (const a of assignments) await s.adjustVehicleStock(a.vehicle_id, 'ITEM-001', { type: 'restock', quantity_delta: '10', reason: 'Test' }, meta);
  ...
  await s.changePatientCaseAssignment(c.patient_case_id, { assignment_id: assignments[1].assignment_id }, meta);
  await s.createInterventionForEncounter(e.encounter_id, treatment, { ...meta, idempotencyKey: 'second' });
  const usage = await s.db.queryAll('SELECT * FROM stock_usage ORDER BY intervention_id');
  assert.deepEqual(usage.map(u => u.vehicle_id), ['AMB-001','AMB-002']);
  assert.ok(usage.every(u => u.patient_case_id === c.patient_case_id));
  assert.equal((await s.getPatientCase(c.patient_case_id)).openemr_encounter_id, e.encounter_id);
  assert.equal((await s.getPatientCase(c.patient_case_id)).openemr_patient_id, 'patient');
});
