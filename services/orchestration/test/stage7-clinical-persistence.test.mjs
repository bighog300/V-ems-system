...
test("migration 009 creates Stage 7 clinical tables", async () => {
  ...
  for (const table of [
    ...
    const row = await db.queryOne(`SELECT name FROM sqlite_master WHERE type='table' AND name='${table}';`);
    assert.equal(row?.name, table);
  }
});

test("demographics preserve unknown identity metadata", async () => {
  ...
  const record = await repository.find("PCR-000001");
...
test("serial observations remain immutable and chronological", async () => {
  const { db, now } = await setup();
  ...
  const records = await repository.list("PCR-000001");
...
test("assessment payloads and timeline events remain structured", async () => {
  const { db, now } = await setup();
  ...
  assert.equal((await assessments.list("PCR-000001"))[0].payload.airway, "patent");
  assert.equal((await timeline.list("PCR-000001"))[0].payload.section_type, "primary_survey");
});

test("medication authorization metadata is retained for audit", async () => {
  const { db, now } = await setup();
  ...
  assert.deepEqual((await repository.find("MED-1")).authorization, { protocol: "TEST-PROTOCOL", version: "1" });
});
