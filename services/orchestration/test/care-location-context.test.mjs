...
test("createEncounterForPatientCase persists optional location context", async () => {
  ...
  const encounter = await service.getPatientCaseEncounter(patientCase.patient_case_id);
...
test("setPatientCaseDisposition persists optional location context", async () => {
  const { service, patientCase, meta } = await setup();
  const disposition = await service.setPatientCaseDisposition(
    patientCase.patient_case_id,
...
test("setPatientCaseDisposition never requires location", async () => {
  const { service, patientCase, meta } = await setup();
  const disposition = await service.setPatientCaseDisposition(
    patientCase.patient_case_id,
...
test("setPatientCaseDisposition rejects an out-of-range longitude", async () => {
  const { service, patientCase, meta } = await setup();
  await assert.rejects(
...
test("setPatientCaseDisposition rejects a negative accuracy", async () => {
  const { service, patientCase, meta } = await setup();
  await assert.rejects(
    () => service.setPatientCaseDisposition(
      patientCase.patient_case_id,
