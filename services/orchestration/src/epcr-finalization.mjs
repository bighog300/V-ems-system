...
async function requireCase(service, patientCaseId) { return service.getPatientCase(patientCaseId); }
async function currentState(service, patientCaseId) {
  return (await service.db.queryOne(`SELECT new_state FROM epcr_lifecycle_events WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY occurred_at DESC, lifecycle_event_id DESC LIMIT 1;` ))?.new_state ?? "draft";
}
async function latestVersion(service, patientCaseId) {
  return row(await service.db.queryOne(`SELECT * FROM epcr_versions WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY version_number DESC LIMIT 1;`));
}
async function snapshot(service, patientCaseId) {
  const patientCase = await requireCase(service, patientCaseId);
  const incident = await service.incidents.findById(patientCase.incident_id);
  const patientLink = (await service.patientLinks.findByPatientCaseId(patientCaseId)) ?? null;
...
async function createVersion(service, patientCaseId, lifecycleState, meta, sourceRevision = "clinical_record") {
  const content = await snapshot(service, patientCaseId), canonical = canonicalJson(content), hash = hashCanonical(content), prior = await latestVersion(service, patientCaseId);
...
export const epcrFinalizationMethods = {
  async assertPatientCaseClinicalMutable(patientCaseId) {
    const state = await currentState(this, patientCaseId);
  ...
  async createEpcrVersion(patientCaseId, payload = {}, meta) {
    await requireCase(this, patientCaseId); const state = await currentState(this, patientCaseId);
    return createVersion(this, patientCaseId, payload.lifecycle_state ?? state, meta, payload.source_revision ?? "manual");
  },
  async listEpcrVersions(patientCaseId) { await requireCase(this, patientCaseId); return this.db.queryAll(`SELECT version_id,patient_case_id,version_number,lifecycle_state,content_hash,hash_algorithm,source_revision,created_at,created_by,correlation_id FROM epcr_versions WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY version_number;`); },
  async getEpcrVersion(patientCaseId, versionId) { await requireCase(this, patientCaseId); const v = row(await this.db.queryOne(`SELECT * FROM epcr_versions WHERE patient_case_id=${sqlValue(patientCaseId)} AND version_id=${sqlValue(versionId)};`)); if (!v) throw new ApiError("NOT_FOUND", "ePCR version not found", 404); return v; },
  async transitionEpcr(patientCaseId, nextState, meta, reason = null, versionId = null) {
    await requireCase(this, patientCaseId); const previous = await currentState(this, patientCaseId);
  ...
  async completeEpcr(patientCaseId, meta) {
    ...
    const version = await createVersion(this, patientCaseId, "crew_complete", meta, "crew_complete"); await this.transitionEpcr(patientCaseId, "crew_complete", meta, meta.reason ?? null, version.version_id); return { version, readiness, lifecycle: await this.getEpcrLifecycle(patientCaseId) };
  },
  async signEpcr(patientCaseId, payload, meta) {
    const state = await currentState(this, patientCaseId), version = await latestVersion(this, patientCaseId);
    ...
    await this.db.execute(`INSERT INTO epcr_signatures (${Object.keys(signature).join(",")}) VALUES (${Object.values(signature).map((v, i) => sqlValue(i === 12 ? json(v) : v)).join(",")});`);
    if (state === "crew_complete") await this.transitionEpcr(patientCaseId, "signed", meta, "signature recorded", version.version_id);
    await audit(this, patientCaseId, "signed", meta, null, { signature_id: signature.signature_id, version_id: version.version_id, record_hash: version.content_hash, signer_role: signature.signer_role });
  ...
  async reviewEpcr(patientCaseId, payload, meta) {
    ...
    const state = await currentState(this, patientCaseId), v = await latestVersion(this, patientCaseId); if (!v || !["submitted", "qa_review"].includes(state)) throw new ApiError("INVALID_STATUS_TRANSITION", "Only submitted ePCRs can enter clinical review", 409);
    if (state === "submitted") await this.transitionEpcr(patientCaseId, "qa_review", meta, payload.comment ?? null, v.version_id);
    const resulting = payload.action === "return_for_correction" ? "returned_for_correction" : payload.action === "finalize" ? "final" : "qa_review";
    if (resulting !== "qa_review") await this.transitionEpcr(patientCaseId, resulting, meta, payload.comment ?? null, v.version_id);
    ...
    await this.db.execute(`INSERT INTO epcr_reviews (${Object.keys(review).join(",")}) VALUES (${Object.values(review).map(sqlValue).join(",")});`); await audit(this, patientCaseId, "reviewed", meta, null, { review_id: review.review_id, action: review.action, version_id: v.version_id });
    return { review, lifecycle: await this.getEpcrLifecycle(patientCaseId) };
  },
  async listEpcrReviews(patientCaseId) { await requireCase(this, patientCaseId); const rows = await this.db.queryAll(`SELECT * FROM epcr_reviews WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY created_at;`); return rows.map(r => ({ ...r, flags: parseJson(r.flags_json, []) })); },
  async createEpcrAmendment(patientCaseId, payload, meta) {
    await requireCase(this, patientCaseId); const base = await this.getEpcrVersion(patientCaseId, payload.base_version_id ?? (await latestVersion(this, patientCaseId))?.version_id); if ((await currentState(this, patientCaseId)) !== "final") throw new ApiError("CONFLICT", "Amendments are only available after finalization", 409);
    ...
    await this.db.withTransaction(async () => {
      await this.db.execute(`INSERT INTO epcr_versions (${Object.keys(version).join(",")}) VALUES (${Object.values(version).map(sqlValue).join(",")});`);
      await this.db.execute(`INSERT INTO epcr_amendments (${Object.keys(amendment).join(",")}) VALUES (${Object.values({ ...amendment, resulting_version_id: version.version_id }).map(sqlValue).join(",")});`);
    });
    await this.transitionEpcr(patientCaseId, "draft", meta, "amendment created", version.version_id);
    await audit(this, patientCaseId, "amended", meta, { version_id: base.version_id }, { amendment_id: amendment.amendment_id, version_id: version.version_id, content_hash: version.content_hash });
    return { amendment: { ...amendment, resulting_version_id: version.version_id }, version: row(version) };
  },
  async listEpcrAmendments(patientCaseId) { await requireCase(this, patientCaseId); const rows = await this.db.queryAll(`SELECT * FROM epcr_amendments WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY created_at;`); return rows.map(r => ({ ...r, before_value: parseJson(r.before_value_json), after_value: parseJson(r.after_value_json) })); },
  async createEpcrQaFlag(patientCaseId, payload, meta) {
    await requireCase(this, patientCaseId); const flag = { flag_id: id("QAF"), patient_case_id: patientCaseId, version_id: payload.version_id ?? (await latestVersion(this, patientCaseId))?.version_id ?? null, flag_type: String(payload.flag_type ?? "").trim(), severity: payload.severity ?? "warning", source: payload.source ?? "manual", raised_at: new Date().toISOString(), raised_by: meta.actorId ?? null, resolved_at: null, resolved_by: null, resolution_note: null, correlation_id: meta.correlationId }; if (!flag.flag_type) throw new ApiError("INVALID_PAYLOAD", "flag_type is required", 400); await this.db.execute(`INSERT INTO epcr_qa_flags (${Object.keys(flag).join(",")}) VALUES (${Object.values(flag).map(sqlValue).join(",")});`); await audit(this, patientCaseId, "qaFlagRaised", meta, null, { flag_id: flag.flag_id, flag_type: flag.flag_type, version_id: flag.version_id }); return flag;
  },
  async listEpcrQaFlags(patientCaseId) { await requireCase(this, patientCaseId); return this.db.queryAll(`SELECT * FROM epcr_qa_flags WHERE patient_case_id=${sqlValue(patientCaseId)} ORDER BY raised_at;`); },
