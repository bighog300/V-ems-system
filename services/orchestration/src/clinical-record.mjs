...
async function appendTimeline(service, record, meta) {
  await service.clinicalTimeline.create({
...
export const clinicalRecordMethods = {
  async getPatientCaseDemographics(patientCaseId) {
    await requiredCase.call(this, patientCaseId);
    return (await this.clinicalDemographics.find(patientCaseId)) ?? null;
  },
  async savePatientCaseDemographics(patientCaseId, payload, meta) {
    const current = await requiredCase.call(this, patientCaseId);
  ...
  async listPatientCaseAssessments(patientCaseId) { await requiredCase.call(this, patientCaseId); return this.clinicalAssessments.list(patientCaseId); },
  async createPatientCaseAssessment(patientCaseId, payload, meta) {
    ...
    if (meta.idempotencyKey) { const existing = await this.idempotency.get("assessment", meta.idempotencyKey); if (existing) { if (existing.request_fingerprint !== fingerprint) throw new ApiError("CONFLICT", "Idempotency key was reused with a different request", 409); return this.clinicalAssessments.find(existing.resource_id); } }
  ...
  async listPatientCaseMedications(patientCaseId) { await requiredCase.call(this, patientCaseId); return this.clinicalMedications.list(patientCaseId); },
  ...
  async setPatientCaseDisposition(patientCaseId, payload, meta) {
    const current = await requiredCase.call(this, patientCaseId); object(payload);
    await this.assertPatientCaseClinicalMutable(patientCaseId);
    const before = await this.clinicalDispositions.find(patientCaseId);
  ...
  async listPatientCaseTimeline(patientCaseId) {
    const current = await requiredCase.call(this, patientCaseId);
    const events = await this.clinicalTimeline.list(patientCaseId);
    return events
