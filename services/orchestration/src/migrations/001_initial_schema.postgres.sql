...
-- SQLite's INTEGER PRIMARY KEY AUTOINCREMENT becomes a Postgres identity
-- column. Nothing reads audit_logs.id or sync_intents.intent_id as an
-- app-level sequence contract (intent_id is surfaced as a plain integer,
-- not relied on for gap-free numbering), so a standard SERIAL is enough —
-- no need to match SQLite's stricter monotonic-only-increasing guarantee.
