...
-- SQLite's printf('%06d', n) becomes lpad(n::text, 6, '0') — Postgres has
-- no printf. Everything else in this migration is standard SQL.
...
SELECT 'PCR-' || lpad(row_number() OVER (ORDER BY i.incident_id)::text, 6, '0'), i.incident_id,1,
