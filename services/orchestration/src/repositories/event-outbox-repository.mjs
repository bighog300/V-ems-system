...
export class EventOutboxRepository {
  ...
  async append(event) {
    await this.db.execute(`INSERT INTO event_outbox (event_id, event_type, occurred_at, source_system, correlation_id, payload_json)
  ...
  async listAll() {
    ...
    const rows = await this.db.queryAll("SELECT * FROM event_outbox ORDER BY occurred_at, rowid;");
