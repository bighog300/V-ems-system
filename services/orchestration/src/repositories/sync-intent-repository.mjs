...
export class SyncIntentRepository {
  ...
  append(intent) {
    this.db.execute(`INSERT INTO sync_intents (target_system, intent_type, entity_type, operation, correlation_id, created_at, status, attempt_count, payload_json)
      VALUES (${sqlValue(intent.target_system)}, ${sqlValue(intent.intent_type ?? intent.operation)}, ${sqlValue(intent.entity_type)}, ${sqlValue(intent.operation)}, ${sqlValue(intent.correlation_id)}, ${sqlValue(intent.created_at)}, 'pending', 0, ${sqlValue(JSON.stringify(intent.payload))});`);
  }
  ...
  listPending(limit = 100) {
    return this.db.queryAll(
      `SELECT * FROM sync_intents WHERE status = 'pending' ORDER BY intent_id LIMIT ${sqlValue(limit)};`
    ).map(mapIntent);
  }
