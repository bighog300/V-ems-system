...
export class VtigerLinkRepository {
  ...
  async findByIncidentId(incidentId) {
    return map(await this.db.queryOne(`SELECT * FROM vtiger_links WHERE incident_id = ${sqlValue(incidentId)};`));
  }

  async findByExternalKey(externalKey) {
    return map(await this.db.queryOne(`SELECT * FROM vtiger_links WHERE external_key = ${sqlValue(externalKey)};`));
  }

  async upsert(link) {
    await this.db.execute(`INSERT INTO vtiger_links
  ...
  async markFailure(incidentId, code, status = "retrying", updatedAt = new Date().toISOString()) {
    await this.db.execute(`UPDATE vtiger_links SET sync_status=${sqlValue(status)}, last_error_code=${sqlValue(code)}, updated_at=${sqlValue(updatedAt)} WHERE incident_id=${sqlValue(incidentId)};`);
  }
}
